...
### 3.4 效果系统（Algebraic Effects）✅ 已完成
```my-lang
effect State {
  get : unit -> int
  set : int -> unit
...
- **GADT** — 广义代数数据类型，构造函数带返回类型约束
- **代数效果（Effects）** — `effect`/`perform`/`handle`/`resume`
- **增量编译** — 模块依赖分析 + 拓扑排序 + 内容哈希缓存
- **AST 缓存** — 使用 Marshal 序列化跳过重复的 lex/parse
- **150+ 个测试全部通过**（22 个测试套件，覆盖所有子系统）

### ⏳ 下阶段目标（按优先级）
1. **LLVM 后端** — 从寄存器字节码生成 LLVM IR
2. **并行编译** — 拓扑层级并行编译，多核利用
3. **细粒度符号依赖** — 函数级增量而非文件级
4. **类型环境缓存** — 缓存模块导出类型签名，跳过完整类型检查
5. **调试器** — 源码级调试
6. **文档生成** — `my-lang doc`
...
4. ~~**代数效果（Effects）**~~ ✅ — effect/perform/handle/resume
5. ~~**GADT**~~ ✅ — 广义代数数据类型，构造函数带返回类型约束
...
3. ~~**增量编译**~~ ✅ — 模块依赖分析 + AST 缓存，跳过未变更模块
4. ~~**LLVM 后端**~~ ⏳ — 从寄存器字节码生成 LLVM IR（下阶段目标）

## 下阶段目标
- **LLVM 后端**：从寄存器字节码生成 LLVM IR，直接生成机器码
- **并行编译**：拓扑层级并行编译，提升多核利用率
- **细粒度符号依赖**：函数级增量编译而非文件级
- **类型环境缓存**：缓存模块导出类型签名，跳过完整类型检查
- **调试器**：源码级调试（断点、单步、查看变量）
- **文档生成**：`my-lang doc` 从注释生成 API 文档

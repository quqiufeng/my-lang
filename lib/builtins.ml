...
let create_builtin_env ctx =
  ...
  ; ( "take",
      VBuiltin
        ( "take",
          fun env -> function
          | VTuple [VInt n; VList items] when n >= 0 ->
              Ok (VList (Core.List.take items n), env)
          | VTuple [VInt n; v] when n >= 0 ->
              Error ("take: 第二个参数需要列表，但得到 " ^ type_of_value v)
          | VTuple [VInt n; _] when n < 0 -> Error "take: 参数不能为负数"
          | VTuple [v; _] -> Error ("take: 第一个参数需要 int，但得到 " ^ type_of_value v)
          | v -> Error ("take: 需要 (int, 列表) 元组，但得到 " ^ type_of_value v) ) )
  ; ( "drop",
      VBuiltin
        ( "drop",
          fun env -> function
          | VTuple [VInt n; VList items] when n >= 0 ->
              Ok (VList (Core.List.drop items n), env)
          | VTuple [VInt n; v] when n >= 0 ->
              Error ("drop: 第二个参数需要列表，但得到 " ^ type_of_value v)
          | VTuple [VInt n; _] when n < 0 -> Error "drop: 参数不能为负数"
          | VTuple [v; _] -> Error ("drop: 第一个参数需要 int，但得到 " ^ type_of_value v)
          | v -> Error ("drop: 需要 (int, 列表) 元组，但得到 " ^ type_of_value v) ) )
  ; ( "find",
      VBuiltin
        ( "find",
          fun env -> function
          | VTuple [f; VList items] ->
              ...
              in
              find_loop items
          | VTuple [f; v] -> Error ("find: 第二个参数需要列表，但得到 " ^ type_of_value v)
      ...
      VBuiltin
        ( "exists",
          ...
          | VTuple [f; VList items] ->
              ...
              in
              exists_loop items
          | VTuple [f; v] -> Error ("exists: 第二个参数需要列表，但得到 " ^ type_of_value v)
      ...
      VBuiltin
        ( "forall",
          ...
          | VTuple [f; VList items] ->
              ...
              in
              forall_loop items
          | VTuple [f; v] -> Error ("forall: 第二个参数需要列表，但得到 " ^ type_of_value v)
